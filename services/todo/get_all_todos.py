...
async def get_all_todos(client: Client, before: Optional[str] = None, after: Optional[str] = None) -> ReturnModel | ErrorModel:
    '''
    ...
    documents = collection.find({ "creationDate": query } if len(query.keys()) > 0 else {}, {"_id": 0}).limit(1000)
    return {
        "status": "OK",
        "result": documents
    }

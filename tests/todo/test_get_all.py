...
def test_get_all():
    ...
    response = client.get(
        "/todo?after=2021-11-06T00:00:00.000Z&before=2021-11-17T23:59:59.999Z"
    )
    assert response.status_code == 200
    assert response.json()["status"] == "OK"
    ...
    response = client.get(
        "/todo?after=1991-01-01T00:00:00.000Z&before=1991-12-31T23:59:59.999Z"
    )
    assert response.status_code == 200
    assert response.json()["status"] == "OK"
...
def test_fail_for_get_all_with_invalid_dates():
    ...
    assert_ko(
        ErrorCode.A02,
        client.get("/todo?before=2021-11-03T16:00:00.000Z&after=202111-T16:00:00.000Z"),
    )
